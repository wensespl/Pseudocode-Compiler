...
class Parser:
    ...
    def p_array_decl_stmt(self, p):
        ...
        p[0] = AST.Array_Declaration(dType, name, elements)
    ...
    def p_assignment_stmt(self, p):
        ...
        var = AST.Variable(dType, var,length)
        p[0] = AST.Assignment("=",dType,var,expr)

    def p_array_assign_stmt(self, p):
        '''assignment_stmt : array_index EQUALS expression'''

        if (p[1].name, self.scope) in self.variable_types:
            p[0] = AST.Assignment("=", self.variable_types[(p[1].name, self.scope)], p[1], p[3])
        else:
    ...
    def p_input_stmt(self, p):
        ...
        if isinstance(p[2], AST.Array_Element):
            name = p[2].name
        else:
            name = p[2]
        ...
        if isinstance(p[2], AST.Array_Element):
            ...
            var = AST.Variable(self.variable_types[(name, self.scope)], p[2],0)
        
        p[0] = AST.Input(var, self.variable_types[(name, self.scope)])
    ...
    def p_function_stmt(self, p):
        ...
        p[0] = AST.Function_Decl(name, args, p[3], dType)
    ...
    def p_expression_arithmetic_binop(self, p):
        ...
        if p[1].dType == None or p[3].dType == None:
            ...
            if p[2] == '+':
                ...
                if isinstance(p[3],AST.Variable):
                ...
                p[0] = AST.BinaryOp(p[2],p[1],p[3],str,total_length)
            ...
            p[0] = AST.BinaryOp(p[2],p[1],p[3],float,0)

        elif p[1].dType == int and p[3].dType == int:
            p[0] = AST.BinaryOp(p[2],p[1],p[3],int,0)
    ...
    def p_expression_unop(self, p):
        ...
        p[0] = AST.UnaryOp(p[2].dType, p[1], p[2], p[2].length)
    ...
    def p_expression_func_call(self, p):
        ...
        if len(p) == 5:
            ...
            if type(p[3]) == list:
                p[0] = AST.Function_Call(p[1], p[3], self.functions[p[1]], 0)
                
            else:
                p[0] = AST.Function_Call(p[1], [p[3]], self.functions[p[1]], 0)
        
        elif len(p) == 4:
            p[0] = AST.Function_Call(p[1], [], self.functions[p[1]], 0)
    ...
    def p_expression_array_val(self, p):
        ...
        if (p[1], self.scope) in self.variable_types:
            p[0] = AST.Array_Element(self.variable_types[(p[1], self.scope)], p[1], p[3], 0)
        else:
            p[0] = AST.Array_Element(None, p[1], p[3], None)
    ...
    def p_expression_var(self, p):
        ...
        if (p[1], self.scope) in self.variable_types:
            ...
            p[0] = AST.Variable(self.variable_types[(p[1], self.scope)],p[1],length)
            
        else:
            p[0] = AST.Variable(None,p[1],0)
