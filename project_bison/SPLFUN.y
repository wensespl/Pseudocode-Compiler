...
function_header: SUBROUTINE VAR {strcpy(scope,lexema);$$=localizaFun(lexema,VAR);} '(' arg_list ')' {TablaFun[$3].nArgs=nArgFun;nArgFun=0;$$=$3;};
arg_list: expr {TablaFun[nFun-1].args[nArgFun++]=$1;}
        | arg_list ',' expr {TablaFun[nFun-1].args[nArgFun++]=$3;};

function_stmt: function_header {generaCodigo(SALTAR,'?','-','-');$$=cx;} stmt_list {TCodigo[$2].a1=cx+1;} ENDSUBROUTINE {TablaFun[$1].iniFun=$2;TablaFun[$1].finFun=nRet;TablaFun[$1].retVal=retVal;sprintf(scope,"%s","");};
return_stmt: RETURN expr ';' {generaCodigo(RETORNO,'?','-','-');nRet=cx;retVal=$2;};
